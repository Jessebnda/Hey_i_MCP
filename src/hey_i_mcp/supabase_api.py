    ...
    def insert_row(
        ...
        try:
            ...
            if not isinstance(row, dict) or not row:
                raise ValueError("row must be a non-empty dictionary.")

            base_url = self._get_base_url()
            api_key = self._get_api_key()

            full_url = f"{base_url}/rest/v1/{quote(normalized_table_name, safe='._')}"
            request_body = json.dumps(row).encode("utf-8")
            ...
            rows = json.loads(raw_body) if raw_body else []
            if not isinstance(rows, list):
                raise RuntimeError("Supabase REST insert response was not a JSON array.")
